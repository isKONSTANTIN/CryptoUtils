...
/**
 * Renders {@link SharePrintLayoutPlanner.PrintPage}s into actual page-sized PNGs, blitting each
...
public final class SharePrintPageRenderer {
    ...
    public static List<BufferedImage> render(List<SharePrintLayoutPlanner.PrintPage> pages) {
        List<BufferedImage> canvases = new ArrayList<>();

        for (SharePrintLayoutPlanner.PrintPage page : pages) {
            ...
            for (SharePrintLayoutPlanner.PlacedImage item : page.items())
